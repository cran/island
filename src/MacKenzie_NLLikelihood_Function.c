...
void program_exit_for_matrix_corruption(int Sp, int Presence, int Time)
{
  ...
  Rprintf("State can only be either 0 or 1\n");
  Rprintf("The program will exit\n");
  Rprintf("Program aborted...");  error("Program aborted");
				      ...
				      int * Transects, int * Time_Index, int n)
{
  ...
  for(i=0; i<n; i++) {
    ...
    for (j=n_0; j<(n_0+Transects[i]); j++) {
      ...
      if( j >= N )
	// error(0,0, "Program aborted");
	{ Rprintf("Program will abort..."); error("Program aborted"); }
    ...
    for(j=0; j<No_of_SPECIES; j++) {
      ...
      for(k=0; k<No_of_Compatible_Dynamics; k++) {
        ...
        if (Presence[0] == 0) {
          ...
          { Rprintf("Number of Transects does not match"); error("Program aborted"); }
        ...
        for(i=1; i<T; i++) {
          ...
          k_0 = Presence[i-1];    if( k_0 != 0 && k_0 != 1 )
            ...
            if( Presence[i] == 0 ) {
              ...
              { Rprintf("Number of Transects do not match"); error("Program aborted"); }
            }
