  ...
  */
  for(i=0; i<No_of_SPECIES; i++)
    if( General_No_of_SAMPLING_TIMES != Data[i]->No_of_TIMES )
		...
		{ Rprintf("Number of Sampling Times does not match: program aborted"); error("Program aborted"); }
  ...
  for(i=0; i<No_of_SPECIES; i++) { /* i-th Partitition:
	...
	for( k=0; k<R[i][j]; k++ ) {
	  ...
	  if( N00 + N01 + N10 + N11 != No_of_TRANSITIONS )
		...
		{ Rprintf("Number of Transititions do not sum up"); error("Program aborted"); }
